...
Images generated by an agent are rendered inline once the response finishes (full resolution in
Kitty, iTerm2, WezTerm, Konsole, and other terminals with graphics support; ANSI block characters
elsewhere), followed by a link to the full-resolution asset. Pass `--no-images` or set
`DIGEN_IMAGES=off` to always print links instead; this happens automatically when output isn't a
